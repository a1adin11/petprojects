...
const CartPage = () => {
  const { data: cartItemsQuery = [] } = useGetCartItemsQuery();
  ...
  const uniqueCartItems = cartItems.filter((item, index, self) => {
    return self.findIndex((otherPizza) => item.id === otherPizza.id) === index;
  });
      ...
      <div className="cart">
        ...
        <div className="cart__bottom">
          <div className="cart__bottom-details">
            ...
            <span>
              Сумма заказа:{" "}
              <b>{cartItems.reduce((sum, item) => (sum += item.price), 0)} ₽</b>
            </span>
          </div>
